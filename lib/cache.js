...
class Cacher {
   ...
   * @param {object|boolean} [payload.mem] 内存缓存配置，传false表示不启用
   * @param {number} [payload.mem.minRedisTtl] 最小可放内存的 redis 过期时间阈值 ms。默认 1000ms，redis.ttl 结果小于 1000ms 的就不会放到内存。0 代表有效 ttl 会全放。
